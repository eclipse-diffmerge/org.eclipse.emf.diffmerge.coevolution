 ...
 * </copyright>
 */
package org.eclipse.emf.diffmerge.bridge.examples.uml.modular.actions;
...
/**
 * An action for running the abstraction operation from the GUI.
 * @author O. CONSTANT
 */
public class UMLBridgeAction implements IObjectActionDelegate {
  ...
  public void run(IAction action_p) {
    ...
    if (context != null) {
      new UMLBridgeJob(context).schedule();
    } else {
      MessageDialog.openError(
          _workbenchPart.getSite().getShell(),
          ModularUMLExampleActivator.getDefault().getLabel(),
          Messages.UMLBridgeAction_IncorrectSelection);
    }
  }
